# Yoio
Um jogo de aprendizagem, que auxilia crianças atipicas com as dificuldades de assimilação de conteúdo escolar. 
Uso de mini-games educativos que estimulam o desenvolvimento cognitivo das mesmas.

# Indentificação do grupo
Nome do Time: Yoio
Turma: 2ºC Desenvolvimento de Sistemas.

# Papéis do Time:

-Product Manager - Lucas Cantelli Medeiros
-Scrum Master - Lucas Konjuski/Eduarda Ayumi Nakano Torres
-Dev Back-End - Eduarda Ayumi Nakano Torres/Lucas Cantelli Medeiros
-Dev Front-End / UX - Bruna Valera Marques
-QA + Documentação - Heloisa Harue Ussami Goncalves

# O Problema que Vamos Resolver
-Qual é a dor? (Uma frase: quem sofre e com o quê.) 
 Crianças atípicas com dificuldades de aprendizado.
 
-Quem é o usuário final?  (Seja específico: não “pessoas”, mas “dona de lanchonete” ou “aluno do ensino médio”.),
 Crianças no processo de aprendizagem e com dificuldades.
  
-Complete a frase: 
 “Nosso software ajuda CRIANÇAS ATÍPICAS a resolver ATIVIDADES COGNITIVAS , eliminando O DESGASTE PSICOLÓGICO ATRELADO AO ESTUDO.”

# O Sistema
-Resumo do sistema:
 Um jogo de aprendizagem, que auxilia crianças atípicas com as dificuldades de assimilação de conteúdos escolares, utilizando ‘mini games’ educativos que estimulam o desenvolvimento cognitivos das mesmas.
 
-Meta do Sprint 1: (A primeira funcionalidade que vocês entregarão nas próximas 2 semanas.)                                                                                                                  Primeira versão da  Primeira interface do aplicativo

#  Link do Trello
https://trello.com/invite/b/69d4e7f5108db27bdbfb7044/ATTIcd2b3344b5919b719acf72961724fd58CB946E33/meu-quadro-do-trello

#  Link do figma
https://www.figma.com/design/QMKLsjGdrOgbToYKZy0H0i/login?node-id=0-1&p=f&t=a0mibV4zGQVvRog5-0

# Backlog do Projeto
| Prioridade | História de usuário | Critérios de aceite |
|------------|---------------------|----------------------|
| Essencial | Como criança, quero abrir o aplicativo e ver a tela inicial de forma clara para que o uso seja intuitivo. | 1. Tela de boas-vindas com o nome da criança. 2. Botão grande de "jogar". 3. Fontes amplas e contraste adequado. |
| Essencial | Como criança, quero escolher entre diferentes mini-games para treinar habilidades variadas sem ficar entediado. | 1. Menu com pelo menos 3 opções de jogos. 2. Cada jogo com cor diferente. |
| Essencial | Como criança, quero jogar um mini-game de associação para praticar vocabulário. | 1. Exibe 4 figuras e 4 palavras. 2. Feedback visual (verde/vermelho). 3. Mostra quantos acertos teve. |
| Essencial | Como criança, quero receber feedback imediato ao acertar ou errar para entender meu desempenho na hora. | 1. Som e animação para acerto. 2. Som suave para erro. |
| Essencial | Como criança, quero avançar para uma próxima fase após completar um jogo para sentir que estou progredindo. | 1. Botão "Próximo" ao final. 2. Fases com níveis mais difíceis. 3. Barra de progresso. |
| Importante | Como criança, quero acumular pontos a cada acerto para ver meu desempenho e me sentir recompensado. | 1. Contador de pontuação visível. 2. Exibe total ao final da fase. |
| Importante | Como criança, quero pausar o jogo a qualquer momento para descansar sem perder o progresso. | 1. Botão de pausa em todas as telas. 2. Opções "Continuar" e "Sair". 3. Progresso é salvo ao sair. |
| Importante | Como criança, quero ter ajustes de dificuldade para jogar no meu ritmo e não me frustrar deixando o jogo. | 1. Opção "Fácil / Médio / Difícil". 2. Padrão é Fácil. |
| Desejável | Como criança, quero personalizar meu avatar para me identificar mais no jogo. | 1. Opções de cor de pele, cabelo, camiseta. 2. Avatar aparece na tela inicial. 3. Alterações salvas automaticamente. |
| Desejável | Como criança, quero ganhar medalhas por sequências de acertos para ter metas extras. | 1. Medalhas como "10 acertos seguidos". 2. Álbum de medalhas no menu. |
| Futuro | Como responsável, quero ver um relatório do progresso para acompanhar o desenvolvimento do aprendizado da criança. | 1. Área restrita com senha. 2. Gráfico de desempenho por jogo. |
| Futuro | Como criança, quero jogar em dupla com um amigo para aprender junto e desenvolver o companheirismo. | 1. Opção "Dois jogadores" no menu. 2. Cada um joga em seu dispositivo. 3. Pontuação combinada ou competitiva. |

<img width="1536" height="822" alt="image" src="https://github.com/user-attachments/assets/2950eddd-71a6-4a33-a70a-758cbb872951" />

## Roteiro do Pitch

**Contexto → Conflito → Solução → Prova → Chamada**

| Etapa | Descrição |
|---|---|
| **Contexto** | Durante o desenvolvimento do projeto, identificamos uma dificuldade presente no processo de aprendizagem de algumas crianças atípicas. A assimilação de conteúdos escolares pode ser difícil, principalmente quando as atividades são muito repetitivas ou pouco interativas. Pensando nisso, buscamos uma maneira de tornar esse processo mais estimulante e agradável. |
| **Conflito** | O principal problema que queremos resolver é a dificuldade de assimilação de conteúdos e o desgaste psicológico relacionado ao estudo. Quando uma criança encontra dificuldades constantemente, repetir o mesmo tipo de atividade pode gerar frustração e fazer com que ela perca o interesse em continuar. Por isso, nosso objetivo é utilizar a interação e a lógica dos jogos para transformar parte desse processo em uma experiência mais dinâmica. |
| **Solução** | Para isso, criamos o **Yoio**, um jogo de aprendizagem que utiliza mini-games educativos para estimular diferentes habilidades cognitivas. A criança poderá escolher entre diferentes atividades, receber feedback imediato, acumular pontos e avançar por fases. Também planejamos diferentes níveis de dificuldade, personalização de avatar e medalhas por conquistas. |
| **Prova** | Para organizar o desenvolvimento, utilizamos o Trello para acompanhar nosso backlog e dividir as funcionalidades por prioridade. Também definimos histórias de usuário e critérios de aceite para orientar o desenvolvimento. Durante o primeiro Sprint, conseguimos transformar a ideia inicial em um protótipo funcional de interface e já estruturamos as próximas funcionalidades do aplicativo. |
| **Chamada** | Nosso objetivo é continuar desenvolvendo o Yoio, começando pelos mini-games e utilizando os feedbacks dos usuários para melhorar a experiência. Queremos que o Yoio seja mais do que um jogo educativo — queremos que ele seja uma ferramenta capaz de tornar o aprendizado mais interativo, estimulante e menos desgastante. Por isso, convidamos vocês a conhecer, testar e dar feedback sobre o nosso projeto. |

## Kanban e indicadores

| Métrica | Resultado |
|---|---:|
| WIP atual | 2 cartões |
| Limite de WIP | 3 cartões |
| Cartões analisados | 12 |
| Lead Time médio | 5,92 dias |
| Cycle Time médio | 4 dias |

## Métricas de validação

- **Formulário:** https://docs.google.com/forms/d/e/1FAIpQLSe5xqfjacheH6Wg59MuBUhoYVtZNgCiIFF7XjKnjffcDqSD2w/viewform?usp=publish-editor
- **Total de respostas:** 6
- **Taxa de interesse:** 83,3% demonstraram interesse na solução
- **Talvez:** 16,7% demonstraram interesse parcial
- **Principais feedbacks:**
  - A proposta foi considerada uma boa alternativa para instituições educacionais e para o aprendizado.
  - Foi apontada a necessidade de tornar o aplicativo mais atrativo para incentivar o público a baixá-lo.
  - Foi sugerida a inclusão de diferentes níveis de dificuldade.
  - Alguns participantes não identificaram defeitos no projeto.

### Principais insights

A validação apresentou uma resposta positiva à proposta do Yoio: **83,3% dos participantes afirmaram que teriam interesse em utilizar a solução**. Os principais pontos de melhoria identificados foram aumentar o poder de atração do aplicativo e oferecer diferentes níveis de dificuldade, permitindo adaptar a experiência às necessidades dos usuários.


## Gif da aplicação
![ Tela inicial e funcionamento](./Design%20inicial/MVP%20Yoio.gif)
